...
# Purpose:      Download zonal stats to Google Sheet by image using GEE
# Python:       3.6
#--------------------------------
...
def main(ini_path=None, overwrite_flag=False):
    ...
    Returns:
        None
    """
    logging.info('\nEarth Engine zonal statistics to Google Sheet (by image)')
    ...
    # else:
    # DEADBEEF - Overwrite always for now
    if True:
    ...
    # Assuming worksheet is called "Landsat"
    logging.debug('    Reading Landsat GSHEET')
    gsheet = gc.open_by_key(ini['EXPORT']['gsheet_id']).worksheet('Landsat')
    gsheet_field_list = gsheet.row_values(1)
    gsheet_df = pd.DataFrame(
        data=gsheet.get_all_values()[1:], columns=gsheet_field_list)


    # Filter the dataframe to the target features (using the zone name)
    # zone_names = [
    #     str(zone_ftr['properties'][ini['INPUTS']['zone_field']]).replace(' ', '_')
    #     for zone_ftr in zones['features']]
    # output_df = gsheet_df[gsheet_df['ZONE_NAME'].isin(zone_names)]


    # Identify all needed tiles based on INI settings/filters
    output_tiles = set()
    # output_df_list = []
    for zone_ftr in zones['features']:
        zone = {}
    ...
    for export_id in sorted(export_ids):
        ...
        # For now, write all data that is returned
        logging.debug('    Writing data to sheet')
        print(gsheet_field_list)
        for row_i, f in enumerate(export_info):
            for k, v in f['properties'].items():
                if k not in gsheet_field_list:
                    continue
                logging.debug(row_i+2, k, gsheet_field_list.index(k)+1, v)
                gsheet.update_cell(row_i+2, gsheet_field_list.index(k)+1, v)
        input('ENTER')
